...
1、安装 go module
> go get -u github.com/lucidapp/wechatsdk/v2

2、从 `github.com/lucidapp/wechatsdk/v2/work/msgaudit/lib` 文件夹下复制 `libWeWorkFinanceSdk_C.so` 动态库文件到系统动态链接库默认文件夹下，或者复制到任意文件夹并在当前文件夹下执行 `export LD_LIBRARY_PATH=$(pwd)`命令设置动态链接库检索地址后即可正常使用
...
import (
	"bytes"
	"fmt"
	"github.com/lucidapp/wechatsdk/v2"
